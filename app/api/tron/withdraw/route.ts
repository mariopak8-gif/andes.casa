...
import { WITHDRAW_FEES } from "@/constants";
...
export async function POST(req: Request) {
  try {
    ...
    const isPasswordLocked = await convex.query(
    ...
    if (isPasswordLocked) {
      ...
      const lockTimeRemaining = await convex.query(
      ...
      const hours = lockTimeRemaining
        ? Math.ceil(lockTimeRemaining / (1000 * 60 * 60))
        : 24;
      console.warn(
        `User ${user._id} attempted withdrawal but transaction password is locked for ${hours} more hours`,
      );
      return NextResponse.json(
        {
    ...
    try {
      ...
      if ((bal.usdt || 0) < amountFloat) {
        ...
        return NextResponse.json(
          {
            error:
              "Server hot wallet has insufficient USDT to process this withdrawal. Please contact support.",
      ...
      if ((bal.trx || 0) < 0.5) {
        ...
        return NextResponse.json(
          {
            error:
              "Server hot wallet has insufficient TRX to pay transaction fees. Please contact support.",
    ...
    try {
      ...
      try {
        ...
        if (destBalance.trx < 1) {
          console.log(
            `⚡ Destination address has insufficient TRX (${destBalance.trx}). Attempting to send 1 TRX for activation...`,
          );
          try {
            const fundTxId = await sendTrx(address, 1);
            console.log(
              `✅ Successfully sent 1 TRX to activate destination: ${fundTxId}`,
            );
          } catch (activateErr: any) {
            console.error(
              `⚠️ Failed to send activation TRX: ${activateErr?.message || activateErr}`,
            );
            console.log(
              `   Proceeding with USDT transfer anyway (may fail if destination account is not activated)`,
            );
          }
        }
      } catch (balanceCheckErr: any) {
        console.error(
          `⚠️ Failed to check destination balance: ${balanceCheckErr?.message || balanceCheckErr}`,
        );
